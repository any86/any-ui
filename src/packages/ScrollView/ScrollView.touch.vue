<template>
    <div class="scroll-view" @touchstart="touchstart" @touchmove.stop.prevent="touchmove" @touchend="touchend">
        <div ref="content" class="content" :class="{'auto-shrink': 2 == touch.status}" :style="{transform: `translate3d(0, ${touch.translateYNew}px, 0) scale(1)`, 'transition-duration': `${duration}ms`}">
            <slot></slot>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        touchmove(e) {
            ...
            this.$emit('input', this.touch.translateYNew);
