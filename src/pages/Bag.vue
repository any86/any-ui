<template>
  <ScrollView v-if="1 === status" v-model="scrollY" class="page-bag">
    ...
    <!-- 购物车列表 -->
    <LayoutGoodsList :dataSource="content.goodsList" :scrollY="scrollY" @remove-goods="refreshLazyLoad">
    </LayoutGoodsList>
    <!-- 推荐 -->
    <LayoutRecommend :dataSource="content.recommend" :scrollY="scrollY" @add-goods="addRecommendGoodsToCart"></LayoutRecommend>
    <!-- 礼品列表 -->
...
import LayoutDiscount from './Bag/Discount'
...
export default {
  ...
  methods: {
    ...
    addRecommendGoodsToCart({ goods, index }) {
      this.content.goodsList.children.push(goods);
      this.content.recommend.children.splice(index, 1);
    },
